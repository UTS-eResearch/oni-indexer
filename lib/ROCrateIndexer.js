  ...
  async mapValue(fieldcf, type, item, field, index_as, value) {
    ...
    if( ! fieldcf ) {
      ...
      if( ! fieldcf['skip'] ) {
        ...
        this.logger.debug(`Facet config for ${field} = ${JSON.stringify(fieldcf['facet'])}`);
        if( fieldcf['facet'] ) {
          const facet = this.makeFacet(fieldcf['facet'], value, this.solr[field]);
          if( ! this.facets[type][field] ) {
            this.logger.error(`No facet config found for ${type}/${field}`);
            this.logger.debug(JSON.stringify(this.facets[type]));
            throw Error(`No facet config for ${field}`);
          }

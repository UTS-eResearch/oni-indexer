      ...
      if( ! fieldcf['skip'] ) {
        ...
        if( fieldcf['load_file'] ) {
          ...
          if( fieldcf['resolve'] ) {
            ...
            this.solr[field] = this.resolveValues(item, field, fieldcf);
...
// so resolveAndFlatten now returns an array
  ...
  resolveValues(item, field, cf) {
    this.logger.debug(`resolveValues for ${item["@id"]} ${field}`);

    const resolved = this.resolveAndFlatten(item, field, cf);
    ...
    if( cf['multi'] ) {
      ...
      if( Array.isArray(resolved) ) {
        if( resolved.length > 1 ) {
          this.logger.info("Got multiple values for ${field} which isn't configured as multi");
        }
        return resolved[0];
      }
    }
  ...
  resolveAndFlatten(item, field, cf) {
    const via = [ { property: field } ];

    if( cf["resolve"]["via"] ) {
      const rvia = _.clone(cf["resolve"]["via"]);
      via.splice(2, 0, ...rvia);
      this.logger.debug(`spliced via ${JSON.stringify(via)}`);
    }

    this.logger.debug(`Resolving: ${item['@id']}.${field}`);

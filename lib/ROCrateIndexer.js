...
Types with errors: ${this.errors.join(', ')}`);
  ...
  async mapItem(cfBase, cf, type, item) {
    ...
    if( cf['_plugins'] ) {
      for( let plugin in cf['_plugins'] ) {
        this.runPlugin(plugin, cf['_plugins'][plugin], item);
      }
    }
  ...
  runPlugin(plugin, dest, item) {
    if( !this.plugins[plugin] ) {
      this.plugins[plugin] = require(path.join(process.cwd(), 'lib/plugins', plugin))
    }
    const result = this.plugins[plugin](item, this.crate);
    this.solr[dest] = JSON.stringify(result); 
  }

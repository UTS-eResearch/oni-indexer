...
Types with errors: ${this.errors.join(', ')}`);
  ...
  async createSolrDocument(jsonld, pathResolver, default_id) {
    ...
    // FIXME - need to lift this out to config

    // for (let item of this.crate.getGraph()) {
    //   if (item["@type"] === "Person") {
    //     if(item['militaryService'] && item['militaryService']['@id']){
    //       const mil = this.crate.getItem(item["militaryService"]["@id"]);
    ...
    //       const o = this.crate.getItem(item["occupation_"]["@id"]);
    //       const s = this.crate.getItem(o["sector"]["@id"])
    //       if (s) {
  ...
  async mapItem(cfBase, cf, type, item) {
    ...
    this.logger.debug(`mapItem ${type} ${JSON.stringify(cf)}`);
    ...
    if( item['@reverse'] && cf['@reverse'] ) {
      for( let field in item['@reverse'] ) {
        if( cf['@reverse'][field] ) {
          this.logger.info(`@reverse lookup on ${field}`);
          // note - pass in cf['@reverse'] rather than cf to mapValues because
          // the @reverse config has an extra layer of nesting 
          await this.mapValues(type, cfBase, cf['@reverse'], field, item['@reverse']);
        }
      }
    }

    for( let field in item ) {
      await this.mapValues(type, cfBase, cf, field, item);
    }
  ...
  // refactoring - I want to pass mapValue an item, but how does that
  // affect filterCf?

  async mapValues(type, cfBase, cf, field, item) {
    const value = item[field];
    const indexValues = this.filterCf(type, cf, field, value);
    for( let i of indexValues ) {
      if( !(i['field'] in cfBase) ) {
        await this.mapValue(item, type, i['field'], i['cf'], i['value'])
      }
    }
  }
  ...
  // mapValue(item, type, field, fieldcf, value)
  //
  ...
  async mapValue(item, type, field, fieldcf, value) {
    const debug = this.debug && this.debug.includes(field);
    this.logger.debug(`mapValue ${field} ${JSON.stringify(fieldcf)}`);
      ...
      if( ! fieldcf['skip'] ) {
        ...
        if( fieldcf['load_file'] ) {
          ...
          if( fieldcf['resolve'] ) {
            ...
            this.solr[field] = this.resolveValues(item, field, fieldcf, value);
  ...
  resolveValues(item, field, cf, value) {
    ...
    if( cf['multi'] ) {
      if( Array.isArray(value) ) {
        return value.map((v) => this.resolveAndFlatten(item, field, cf, v));
      } else {
        return [ this.resolveAndFlatten(item, field, cf, value) ];
      }
    } else {
      if( Array.isArray(value) ) {
        return this.resolveAndFlatten(item, field, cf, value[0]);
      } else {
        return this.resolveAndFlatten(item, field, cf, value);
      }
    }
  ...
  resolveAndFlatten(item, field, cf, value, solr) {
    if( !('@id' in value ) ) {
    ...
    if( cf["resolve"]["via"] ) {
      via.push(_.clone(cf["resolve"]["via"]));
    }

    this.logger.debug(`Resolving ${field} in ${JSON.stringify(item)} via ${JSON.stringify(via)}`)
    const target = this.crate.resolve(item, via);

    this.logger.debug(`target = ${JSON.stringify(target)}`)

    if( !target ) {
      this.logger.warn(`item resolution failed for ${field}`);
      return '';
    }
    ...
    const resolved = {
      "@id": target['@id'],
      display: target[cf['resolve']['display']],
      search: this.convertSearch(target, cf['resolve']['search'])
    };
  ...
  resolveAndFlatten_old(cf, value, solr) {
    ...
    this.logger.debug(`resolveAndFlatten cf ${JSON.stringify(cf)} value ${JSON.stringify(value)}`);
  ...
  makeFacet(cf, raw, resolved) {
    ...
    if( cf['tokenize'] ) {
      if( raw ) {
        ...
        this.logger.info(`About to tokenize "${raws}" which is a ${typeof(raws)}`);

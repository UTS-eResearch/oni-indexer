      ...
      _.each(typecf, ( fieldcf, field ) => {
        ...
        if( Array.isArray(fieldcf) ) {
          for( let altcf of fieldcf ) {
            if( altcf['match'] ) {
              const filterId = `${type}.${altcf['index_as']}`;
              this.itemFilters[filterId] = this.compileFilter(altcf['match']);
              this.logger.debug(`Compiled match filter for ${filterId}`);
            }
          }
        }
      });
...
Types with errors: ${this.errors.join(', ')}`);
  ...
  async mapItem(cfBase, cf, type, item) {
    ...
    this.logger.debug(`item = ${JSON.stringify(item, null, 2)}`);
    for( let field in item ) {
      this.logger.info(`Mapping values for ${field}`);
      ...
      if( ! fieldcf['skip'] ) {
        ...
        if( fieldcf['load_file'] ) {
          ...
          if( fieldcf['resolve'] ) {
            ...
            this.solr[field] = this.resolveValues(item, field, index_as, fieldcf);
        ...
        this.logger.debug(`Facet config = ${JSON.stringify(fieldcf['facet'])}`);
        if( fieldcf['facet'] ) {
...
// so resolveAndFlatten now returns an array
    ...
    if( cf['multi'] ) {
      ...
      if( Array.isArray(resolved) ) {
        if( resolved.length > 1 ) {
          this.logger.warn(JSON.stringify(item, null, 2));
          this.logger.warn(JSON.stringify(resolved, null, 2));
          this.logger.warn(`${item['@id']}.${field} has multiple values, isn't configured as multi`);
        }
        return resolved[0];
      }
    }
  ...
  resolveAndFlatten(item, field, index_as, cf) {
    ...
    const matchId = `${this.currentType}.${index_as}`;
    ...
    if( matchId in this.itemFilters ) {
      ...
      this.logger.debug(`fuckn - ${via[lasti]['matchFn']}`);
    }

    via[0]['blah'] = "I hate javascript";

    this.logger.debug(`via = ${JSON.stringify(via, null, 2)}`);

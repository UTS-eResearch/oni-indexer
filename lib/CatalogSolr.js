...
Types with errors: ${this.errors.join(', ')}`);
  ...
  async mapValue(item, type, field, fieldcf, value) {
    const debug = true //( this.debug && this.debug.includes(field) );
...
//        const ivalues = values.filter((v) => typeof(v) === 'object' && matcher(v));
  ...
  // FIXME - this isn't working for via links because value is already an id pair
  // ie the ro-crate resolve expects to be called like resolve(parent_item, [ {property}])
  // but this is being called with the value (location) - how did it get this far?
  ...
  resolveAndFlatten(cf, value, solr) {
    ...
    if( cf["resolve"]["via"] ) {
      ...
      this.logger.info(`resolveAndFlatten via ${JSON.stringify(via)}`);
      this.logger.info(`resolveAndFlatten value ${JSON.stringify(value)}`);
      item = this.crate.resolve(value, via);
    } else {

  ...
  async createSolrDocument(jsonld, pathResolver, default_id) {
    ...
    for (let item of this.crate.getGraph()) {
      if (item["@type"] === "Person") {
        ...
        if(item['hasFile'] && item['hasFile']['@id']){
          const f = this.crate.getItem(item["hasFile"]["@id"]);
          item.image = [];
          if(Array.isArray(f)) {
            for (let fi of f) {
              item.image.push(fi);
        ...
        if(item['this.id'] && item['this.id']['@id']){
          const p = this.crate.getItem(item["this.id"]["@id"]);
          if(p['hasFile'] && p["hasFile"]["@id"]){
            item.pdf = p["hasFile"]["@id"];
          }
        }
      }
    }
...
//        const ivalues = values.filter((v) => typeof(v) === 'object' && matcher(v));
  ...
  resolveAndFlatten(cf, value, solr) {
    this.logger.debug(`resolveAndFlatten ${JSON.stringify(cf)}`);
    if( !('@id' in value ) ) {
    ...
    this.logger.debug(`Resolving item`);
  ...
  chaseItem(item, via) {
    ...
    if( !via || via.length === 0 ) {
      this.logger.debug(`Reference caught: ${JSON.stringify(lookup)}`);
      return lookup;
    }
    const inter = via.shift();
    this.logger.debug(`Chasing reference via ${inter}`);
    if( !lookup[inter] ) {

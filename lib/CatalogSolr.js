...
Types with errors: ${this.errors.join(', ')}`);
  ...
  async mapValue(type, ofield, fieldcf, value) {
    const debug = ( this.debug && this.debug.includes(ofield) );
    ...
    if( ! fieldcf ) {
      ...
      this.solr[ofield] = this.unwrap(value);
      if( debug ) {
        this.logger.debug(`field ${ofield} copy ${value}`);
      }
    } else {
      const field = fieldcf['index_as'] || ofield;
      if( ! fieldcf['skip'] ) {
        ...
        if( debug ) {
          this.logger.debug(`Solr field for ${field}: ${JSON.stringify(this.solr[field])}`);
        }
        // make facets - these can be based on raw or resolved values depending
        // on the faceting rule, so pass both in
        if( fieldcf['facet'] ) {
          this.logger.debug(`Faceting for ${type} ${field}`);

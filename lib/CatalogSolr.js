...
class CatalogSolr {
    ...
    _.each(cf, ( condition, field ) => {
      if( typeof condition === 'object' ) {
        if( condition['re'] ) {
          ...
          fs.push( ( item ) => item[field].match(re) );
        } else {
          console.error("Unknown filter type in " + JSON.stringify(condition) );
        }
      } else {
        fs.push( (item) => item[field] === condition )
      }
    })
  ...
  // item has to match all fields to pass.
  // if no filters defined, return true

  matchFilter(filters, item) {
    if( filters ) {
      _.each(filters, ( value, field ) => {
        if( item[field] && item[field] !== value ) {
          console.log("Item " + JSON.stringify(item));  
          console.log(`failed filter: ${field} ${item[field]} ${value}`);
          return false;
        }
      })
      return true;

...
Types with errors: ${this.errors.join(', ')}`);
  ...
  compileFilter(cf) {
    console.log(`compileFilter ${JSON.stringify(cf)}`);
    const fs = [];
    _.each(cf, ( condition, field ) => {
      if( typeof condition === 'object' ) {
        if( condition['re'] ) {
          ...
          fs.push(f);
          console.log(`- RE ${field} ${JSON.stringify(condition)}`);
        } else if ( condition['is_root'] ) {
          console.log(`- IS_ROOT ${field} ${JSON.stringify(condition)}`);
          fs.push((item) => {
            if( this.root ) {
  ...
  mapItem(cfBase, cf, type, item) {
    ...
    _.each(item, ( value, field ) => {
      ...
      _.each(indexValues, (i) => {
        if( !(i['field'] in cfBase) ) {
          this.mapValue(type, i['field'], i['cf'], i['value'])
        }
      });
    });
  ...
  getIndexCf(type, cf, field, value) {
    if( !Array.isArray(cf[field]) ) {
      ...
      _.each(cf[field], (indexCf) => {
        ...
        const ivalues = values.filter((v) => matcher(v));
        console.log(`MATCH-> ${JSON.stringify({matcher: indexCf['match'], input: values, output:ivalues}, null, 4)}`);
        if( ivalues.length > 0 ) {
          indexable.push({

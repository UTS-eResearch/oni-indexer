    ...
    _.each(this.config['types'], ( typecf, type ) => {
      ...
      _.each(typecf, ( fieldcf, field ) => {
        ...
        if( fieldcf['facet'] ) {
          ...
          this.facets[type][field] = {
            facetField: this.facetFieldName(type, field, fieldcf)
          };
          this.logger.debug(`facet field name ${type} ${field} ${this.facets[type][field]['facetField']}`);
        }
        if( field === 'licence' ) {
    ...
    if( ! fieldcf ) {
      ...
      if( ! fieldcf['skip'] ) {
          ...
          if( fieldcf['resolve'] ) {
            ...
            for (let val of vals) {
              try {
                ...
                this.logger.error(`Resolution error: ${e.message} ${val}`);
              }
            }
            if( debug ) {
        ...
        if( fieldcf['facet'] ) {
          ...
          if( !facet ) {
            this.logger.error(`Empty value for facet ${facetField} - check config`);
          }

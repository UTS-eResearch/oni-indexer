...
//        const ivalues = values.filter((v) => typeof(v) === 'object' && matcher(v));
  ...
  resolveAndFlatten(cf, value, solr) {
    ...
    this.logger.debug(`Resolving item from ${JSON.stringify(value)}`);

    let item;

    if( cf["resolve"]["via"] ) {
      item = this.crate.resolve(value, cf["resolve"]["via"] || []);
    } else {
      item = this.crate.getItem(value['@id']);
    }

    if( !item ) {
      this.logger.warn(`Resolve failed for ${JSON.stringify(value)}`)
      return '';
    }

    ...
    for (const item of items) {
      ...
      if (item['@id'] !== rootOrigId) {
        ...
        for (let type of Object.keys(cfTypes)) {
          //this.logger.info(`Looking for ${type} in ${JSON.stringify(types)}`);
          ...
          if( fieldcf['resolve'] ) {
            ...
            for (let val of vals) {
              try {
                ...
                this.logger.warn(`Resolution error: ${e.message} ${val}`);
              }
            }
            if( debug ) {
...
//        const ivalues = values.filter((v) => typeof(v) === 'object' && matcher(v));
  ...
  resolveValues(cf, value) {
    ...
    if( cf['multi'] ) {
      if( Array.isArray(value) ) {
        ...
        return [ this.resolveAndFlatten(cf['resolve'], value) ];
      }
    } else {
      if( Array.isArray(value) ) { 
        return this.resolveAndFlatten(cf['resolve'], value[0]);
      } else {
        return this.resolveAndFlatten(cf['resolve'], value);
      }
    }
  ...
  resolveAndFlatten(cf, value, solr) {
    ...
    const item = this.crate.getItem(value['@id']);

    if( !item ) {
      return this.convertError(`@id ${value['@id']} not found`);
    }

    // TODO: this is where lookups have to be normalised

    if( !cf['resolve'] ) {
      return this.convertError(`Attempt to resolve an item without a resolve config`);
    }

    if( ! cf['resolve']['search'] ) {
      return this.convertError(`Resolve config doesn't have search value`);
    }
  ...
  makeFacet(cf, raw, resolved) {
    ...
    if( cf['tokenize'] ) {
      if( raw ) {
        return raw.split(RegExp(cf['tokenize']['delim']));
      } else {
        return [];
      }
    }
  ...
  convertError(message) {
    const wrapped = `convert Error: ${message}`;
    this.logger.error(this.rootId + ' ' + wrapped);
    return wrapped;
  }
